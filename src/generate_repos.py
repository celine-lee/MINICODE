...
class Repo:
    ...
    def evaluate(self):
        ...
        result = {}
        try:
            if os.path.exists(os.path.join(self.repo_path, "test_output.txt")): os.remove(os.path.join(self.repo_path, "test_output.txt"))
...
def propose_libraries(agent, args):
    ...
    while len(new_libraries) < args.num_new_generations:
        ...
        for block in file_blocks[1:]:
            ...
            if not library_name:
                continue
            file_path = Path(WRITE_REPOS_TO_FOLDER) / file_path
